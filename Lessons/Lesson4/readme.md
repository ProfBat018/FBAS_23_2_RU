...
docker build -t <имя образа>:<тег> -f <путь к докерфайлу>
...
docker run --name <имя контейнера> --rm -it <имя образа> --port 1433:1433
```

К примеру в таком режиме мы запускаем MsSQL сервер, чтобы мы могли использовать его терминал для выполнения команд.
